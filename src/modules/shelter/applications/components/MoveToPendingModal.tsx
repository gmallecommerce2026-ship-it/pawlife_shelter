...
export const MoveToPendingModal: React.FC<MoveToPendingModalProps> = ({
  ...
  // LOGIC NGHIỆP VỤ: Quản lý danh sách Tag động
  ...
  // LOGIC NGHIỆP VỤ: Quản lý danh sách Ghi chú động
  ...
  // Thêm Ghi chú mới - Gọi API thực sự tới BE
  const handleAddNote = async () => {
    ...
    try {
      ...
      const newNoteObj: ApplicationNote = {
        ...
        authorName:
          addedNote?.author?.name ||
          addedNote?.author?.fullName ||
          'Staff Member',
        authorAvatar:
      ...
      // GỌI REFRESH ĐỂ TRANG CHA CẬP NHẬT DỮ LIỆU TỪ BACKEND NGAY LẬP TỨC
  ...
  // Submit chuyển trạng thái
  const handleSubmit = async () => {
    try {
      await applicationService.updateStatus(application.id, 'PENDING', noteInput);
      onSubmit({
        reviewNote: noteInput || 'Đã chuyển đơn sang Đang xem xét',
        tags,
        notes,
      });
    } catch (error) {
          ...
          <div className="flex gap-5 mb-8 mt-2">
            ...
            <div className="flex flex-col justify-center">
              ...
              <button className="flex items-center gap-2 text-gray-600 hover:text-[#E89B5A] transition-colors text-[13px]">
                <Download size={14} />
                <u>Tải xuống <span className="font-semibold">{(application.fullName || application.user?.name || "Maria").split(' ')[0]} - Application.pdf</span></u>
              </button>

              {/* Lồng ghép Thẻ Pet Mini */}
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-3">
              ...
              <button
                ...
                className="text-gray-400 hover:text-gray-600 text-[12px] flex items-center gap-1"
              >
                + thẻ
              </button>
            </div>
              ...
              <div className="flex flex-col gap-5">
                ...
                {notes.map((note) => (
                  <div key={note.id} className="flex gap-3">
                    <img
                      src={note.authorAvatar || note.author?.avatarUrl || "https://images.unsplash.com/photo-1573865526739-10659fec78a5?q=80&w=100"}
                      className="w-8 h-8 rounded-full object-cover shrink-0"
                      alt="Staff"
                    />
                    <div className="flex flex-col w-full">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-bold text-[13px] text-gray-900">{note.authorName || note.author?.name || 'Staff Member'}</span>
