...
// ---------------------------------------------------------------------------
// Upload 1 file lên R2 qua presigned URL — cùng pattern với useShelterProfileStore.
// Backend (/shelter-dashboard/pets) chỉ nhận JSON với images: string[] (URL),
// KHÔNG nhận multipart/form-data (controller không có FilesInterceptor),
// nên phải upload ảnh trước rồi mới gửi URL lên.
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// Loại bỏ các field optional có giá trị rỗng ('', null) trước khi gửi lên
// backend. Lý do: @IsOptional() của class-validator chỉ bỏ qua validate khi
// giá trị là undefined/null, KHÔNG bỏ qua chuỗi rỗng "". Nếu form để mặc định
// size/gender/status = '' (chưa chọn), gửi thẳng lên sẽ bị @IsEnum() reject.
// ---------------------------------------------------------------------------
function sanitizePayload<T extends Record<string, any>>(values: T): T {
  const cleaned: Record<string, any> = {};
  Object.entries(values).forEach(([key, val]) => {
    if (val === '' || val === null) return; // bỏ hẳn field này, để backend coi như "không gửi"
    cleaned[key] = val;
  });
  return cleaned as T;
}

// Các field enum backend yêu cầu UPPERCASE tuyệt đối (SMALL/MEDIUM/LARGE,
// MALE/FEMALE/UNKNOWN, AVAILABLE/PENDING/ADOPTED...). Form UI có thể gửi
// "Medium", "Male"... (do label hiển thị) nên cần chuẩn hoá lại trước khi gửi,
// thay vì sửa rải rác ở từng component form.
...
// Payload từ form dùng field "adoptionRequirements" (mảng object đầy đủ để
// hiển thị UI), nhưng backend DTO chỉ nhận "adoptionRequirementKeys" (mảng
// string key). Map lại đúng tên + shape trước khi gửi.
...
const usePetStoreBase = create<PetState & PetActions>()((set, get) => ({
  ...
  setFilter: (patch) => {
    ...
    // Đổi bộ lọc/tìm kiếm thì luôn quay về trang 1, trừ khi patch tự set page
  ...
  fetchPets: async (override) => {
    ...
    try {
      ...
      toast.error('Không tải được danh sách pet. Vui lòng thử lại.');
    } finally {
      set({ isLoading: false });
    }
  },
